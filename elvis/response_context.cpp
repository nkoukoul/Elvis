...
void HttpResponseCreator::CreateResponse(std::shared_ptr<ClientContext> c_ctx) const
{
    std::string status;
        ...
        c_ctx->m_HttpHeaders["Upgrade"] == "websocket")
    {
        ...
        sec_websocket_key = generate_ws_key(c_ctx->m_HttpHeaders["Sec-WebSocket-Key"], m_CryptoManager);
...
void WebsocketResponseCreator::CreateResponse(std::shared_ptr<ClientContext> c_ctx) const
{
    int payload_len;
