...
namespace Elvis
{
  ...
  public:
    ...
    virtual std::list<std::unordered_map<std::string, std::string>>
    Deserialize(std::string &&input) const = 0;
  };
  ...
  public:
    ...
    Deserialize(std::string &&input) const override;
  };
  ...
  public:
    ...
    void
    setValidateStrategy(std::unique_ptr<IValidateStrategy> validateStrategy)
    {
      this->m_ValidateStrategy = std::move(validateStrategy);
    }

    std::list<std::unordered_map<std::string, std::string>>
    DoDeserialize(std::string &&input) const
    {
    ...
    void setDeserializeStrategy(
        std::unique_ptr<IDeserializeStrategy> deserializeStrategy)
    {
  ...
  public:
    JSONContext()
    {
      setDeserializeStrategy(
          std::make_unique<DeserializeStrategy>()); // default for now
      setValidateStrategy(
          std::make_unique<ValidateStrategy>()); // default for now
    }
  };
} // namespace Elvis
