...
#ifdef DEBUG
  std::cout << "ValidateStrategy::Validate: "
            << (checker.empty() ? "valid" : "invalid") << "\n";
#endif
  return checker.empty();
}

std::list<std::unordered_map<std::string, std::string>>
Elvis::DeserializeStrategy::Deserialize(std::string &&input) const
{
#ifdef DEBUG
  ...
  int i = 0;
  bool key_turn = true, value_turn = false, count_started = false,
       isString = false;
  ...
  while (i < input.size())
  {
    ...
    { // find quotes
      ...
      { // end of key
        end_of_key = i - 1;
        int key_length =
            end_of_key - start_of_key + 1; // length of value in chars
    ...
    {      // start of value, not part of key/value
      ...
      { // value is list
        ...
        end_of_value = i;
        int value_length =
            end_of_value - start_of_value + 1; // length of value in chars
