...
namespace Elvis
{
  ...
  public:
    ...
    void SetRoute(std::string url, std::string request_type,
                  std::unique_ptr<IController> controller)
    {
      if (m_RouteMap.find(url) == m_RouteMap.end())
      {
        std::unordered_map<std::string, std::unique_ptr<IController>>
            request_types;
        request_types.insert(
            std::move(std::make_pair(request_type, std::move(controller))));
        m_RouteMap.insert(
            std::move(std::make_pair(url, std::move(request_types))));
      }
      else if (m_RouteMap[url].find(request_type) == m_RouteMap[url].end())
      {
        m_RouteMap[url].insert(
            std::move(std::make_pair(request_type, std::move(controller))));
      }
      else
      {
        std::cout << url << " has already a request type " << request_type
                  << " registered\n";
      }
      return;
    }
    ...
    {
      return m_RouteMap.find(url) != m_RouteMap.end() &&
             m_RouteMap[url].find(request_type) != m_RouteMap[url].end();
    }
