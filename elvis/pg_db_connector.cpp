...
{
public:
  pg_connector(std::string dbname, std::string user, std::string password,
               std::string hostaddr, std::string port)
      : dbname_(dbname), user_(user), password_(password), hostaddr_(hostaddr),
        port_(port)
  {
    conn_info_ = "dbname = " + dbname_ + " user = " + user_ +
                 " password = " + password_ + " hostaddr = " + hostaddr_ +
                 " port = " + port_;

    pg_connection_ = std::make_shared<pqxx::connection>(conn_info_);
    if (pg_connection_->is_open())
    {
      std::cout << "Opened connection to database successfully: "
                << pg_connection_->dbname() << "\n";
    }
    else
    {
...
public:
  ...
  void RetrieveModel(std::string query) override
  {
    ...
    for (pqxx::result::const_iterator c = query_result.begin();
