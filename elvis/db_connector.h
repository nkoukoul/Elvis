...
namespace Elvis
{
template <class D, class R> class DBConnector
{
public:
...
public:
    ...
    MockConnector(std::string dbname, std::string user, std::string password, std::string hostaddr, std::string port)
    {
    }
...
public:
    ...
    template <class I_DB_C> std::unique_ptr<I_DB_C> ExtractDBConnectorFromPool();

    template <class I_DB_C> void ReturnDBConnectorToPool(std::unique_ptr<I_DB_C> db_c);
};

template <class DB_C> class DBManager : public IDBManager
{
public:
...
template <class I_DB_C> std::unique_ptr<I_DB_C> IDBManager::ExtractDBConnectorFromPool()
{
    return static_cast<DBManager<I_DB_C>*>(this)->ExtractDBConnectorFromPool();
}

template <class I_DB_C> void IDBManager::ReturnDBConnectorToPool(std::unique_ptr<I_DB_C> db_c)
{
