...
namespace Elvis
{
  ...
  public:
    ...
    MockConnector(std::string dbname, std::string user, std::string password,
                  std::string hostaddr, std::string port) {}

    int Select(std::string sql, std::string &mR) const { return 0; }

    int Insert(std::string sql) const { return 0; }

    int Update(std::string sql) const { return 0; }
  };
  ...
  public:
    DBManager(int connection_num)
    {
      ...
      {
        m_DBConnectionPool.emplace_back(std::make_unique<DB_C>(
  ...
  {
    static_cast<DBManager<I_DB_C> *>(this)->ReturnDBConnectorToPool(
        std::move(db_c));
  }
