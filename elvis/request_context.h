...
{
public:
    WebsocketRequestParser(std::unique_ptr<WebsocketResponseContext> wsResponseContext);
...
public:
    explicit HttpRequestContext(
        std::unique_ptr<HttpResponseContext> httpResponseContext,
        std::shared_ptr<RouteManager> routeManager)
        : IRequestContext(std::make_shared<HttpRequestParser>(std::move(httpResponseContext), routeManager))
    {
    }
};
...
{
public:
    WebsocketRequestContext(std::unique_ptr<WebsocketResponseContext> wsResponseContext)
        : IRequestContext(std::make_shared<WebsocketRequestParser>(std::move(wsResponseContext)))
    {
    }
};
} // namespace Elvis
