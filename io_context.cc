...
tcp_server::tcp_server(std::string ipaddr, int port, std::unique_ptr<i_request_context> req, std::unique_ptr<i_response_context> res, app *ac) : ipaddr_(ipaddr), port_(port), req_(std::move(req)), res_(std::move(res)), ac_(ac)
{
...
void tcp_server::handle_connections()
{
  ...
  if (client_socket < 0)
  {
    ...
    return ac_->e_q_->produce_event<std::function<void()>>(std::move(std::bind(&tcp_server::handle_connections, ac_->http_ioc_.get())));
  }
  std::cout << "socket " << client_socket << " connected" << std::endl;
  return ac_->e_q_->produce_event<std::function<void()>>(std::move(std::bind(&io_context::do_read, ac_->http_ioc_.get(), client_socket)));
}
...
websocket_server::websocket_server(std::string ipaddr, int port, std::unique_ptr<i_request_context> req, std::unique_ptr<i_response_context> res, app *ac) : ipaddr_(ipaddr), port_(port), req_(std::move(req)), res_(std::move(res)), ac_(ac)
{
