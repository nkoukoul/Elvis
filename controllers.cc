...
std::string file_get_controller::run(std::unordered_map<std::string, std::string> &&deserialized_input_data, app *ac)
{
  ...
  if (!ac->app_cache_pool[t_id]->find<std::string, std::string>(filename)){
    ac->app_cache_pool[t_id]->insert<std::string, std::string>(std::make_pair(filename, std::move(ac->uc_->read_from_file("", filename))));
  }
  //std::pair<std::string, std::string> k_v_pair = ac->app_cache_->[filename];
  return (*(ac->app_cache_pool[t_id])).operator[]<std::string, std::string>(filename).second;
  //return {};
}
...
std::string file_post_controller::run(std::unordered_map<std::string, std::string> &&deserialized_input_data, app *ac)
{
  ...
  fm->repr();
  std::thread::id t_id = std::this_thread::get_id();
  ac->app_cache_pool[t_id]->insert<std::string, std::string>(std::make_pair(fm->get_filename(), std::move(ac->uc_->read_from_file("", fm->get_filename()))));
  ac->app_cache_pool[t_id]->state();
  return {};
}
